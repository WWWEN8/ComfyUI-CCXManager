...
class GitHubRepoUpdater:
    ...
    def update_all_repos(self, force_override=False):
        ...
        for repo_info, (success, message) in results:
            ...
            if success:
                ...
                print("[CCXManager Updater] SD-PPP节点更新成功")
            else:
                result_msg = f"❌ {node_name}: {message}\n"
    ...
    def run_auto_update(self, force_override=False, force_update=False):
        ...
        if not self.config.get("auto_update_on_start", True):
            print("[CCXManager Updater] Comfyui侧自动更新SD-PPP[更新禁用]")
            return 0, "自动更新已禁用"
        ...
        print("[CCXManager Updater] 正在SD-PPP节点检查更新...")
...
class CCXRepoUpdaterNode:
    ...
    def process(self, repo_url, auto_update, force_override):
        ...
        # 先检测更新状态
        print(f"[CCXManager Updater] Comfyui侧自动检测{node_name} 仓库的更新状态")
        ...
        if repo_info:
            ...
            if has_update and auto_update:
                # 有更新且开启自动更新，执行更新
                print(f"[CCXManager Updater] 发现{node_name} 的更新，开始更新...")
                ...
                if result.startswith("本地git检查成功"):
                    status_message = f"{node_name} 检查成功，{update_status}"
                else:
...
def auto_check_for_repo_updates():
    ...
    try:
        ...
        # 在控制台显示自动更新状态
        print(f"[CCXManager Updater] Comfyui侧自动更新SD-PPP[{'启动成功' if auto_update_status else '更新禁用'}]")
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "CCXRepoUpdaterNode": "Comfyui side automatic update SD-PPP"
}
