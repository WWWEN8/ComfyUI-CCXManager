...
class QueueManager {
  ...
  initializeHooks() {
    ...
    app.queuePrompt = async function() {
      queueManager.processingQueue = true;
      queueManager.eventManager.dispatchEvent("queue");
      try {
        await originalQueuePrompt.apply(app, [...arguments]);
      } finally {
        queueManager.processingQueue = false;
        queueManager.eventManager.dispatchEvent("queue-end");
      }
    };

    app.graphToPrompt = async function() {
      queueManager.eventManager.dispatchEvent("graph-to-prompt");
      let promise = originalGraphToPrompt.apply(app, [...arguments]);
      await promise;
      queueManager.eventManager.dispatchEvent("graph-to-prompt-end");
      return promise;
    };

    api.queuePrompt = async function(index, prompt) {
      // 创建局部副本，避免异步执行时queueNodeIds被重置导致数据丢失
      const localQueueNodeIds = [...(queueManager.queueNodeIds || [])];
      
      // 修复：检测到有 CCXGroupExecutorSender 输出节点时，只保留它
      if (prompt.output && prompt.nodes) {
        let hasSenderNode = false;
        let senderNodeId = null;
        
        for (const outputNodeId in prompt.output) {
          const nodeData = prompt.nodes[outputNodeId];
          if (nodeData && nodeData.class_type === "CCXGroupExecutorSender") {
            hasSenderNode = true;
            senderNodeId = outputNodeId;
            break;
          }
        }

        // 只在没有明确指定输出节点ID时才进行此修改
        // 避免与CCXGroupExecutor的自定义执行逻辑冲突
        if (hasSenderNode && senderNodeId && !localQueueNodeIds.length) {
          console.log(`[QueueManager] 检测到 CCXGroupExecutorSender 节点，只保留它作为输出节点`);
          // 只保留 sender 节点
          const oldOutput = prompt.output;
          prompt.output = {};
          prompt.output[senderNodeId] = oldOutput[senderNodeId];
        }
      }

      if (localQueueNodeIds.length && prompt.output) {
        const oldOutput = prompt.output;
        let newOutput = {};
        for (const queueNodeId of localQueueNodeIds) {
          queueManager.recursiveAddNodes(String(queueNodeId), oldOutput, newOutput);
        }
        prompt.output = newOutput;
      }
      queueManager.eventManager.dispatchEvent("comfy-api-queue-prompt-before", {
        workflow: prompt.workflow,
        output: prompt.output,
      });
      const response = originalApiQueuePrompt.apply(api, [index, prompt]);
      queueManager.eventManager.dispatchEvent("comfy-api-queue-prompt-end");
      return response;
    };
