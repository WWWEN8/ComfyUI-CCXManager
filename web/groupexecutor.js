...
class CCXGroupExecutorNode extends BaseNode {
    static type = "🎈CCXGroupExecutor";
    static title = "🎈CCX Group Executor";
    static category = "🎈LAOGOU/Group";
    static _category = "🎈LAOGOU/Group";
    constructor(title = CCXGroupExecutorNode.title) {
        super(title, null);
    ...
    async getQueueStatus() {
        try {
            ...
            // 修复：在获取队列状态失败时，返回队列正在运行的状态
            // 这样可以避免waitForQueue方法立即返回，从而避免重复执行
            return {
                isRunning: true,
                isPending: false,
                runningCount: 1,
...
app.registerExtension({
    name: "CCXGroupExecutor",
    registerCustomNodes() {
        CCXGroupExecutorNode.setUp();
    }
});