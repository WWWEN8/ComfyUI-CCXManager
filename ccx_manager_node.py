...
class CCXManager:
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                "source_path": ("STRING", {"default": config.get("source_path", ""), "placeholder": "CCX文件所在路径"}),
                "target_path": ("STRING", {"default": config.get("target_path", ""), "placeholder": "目标解压路径"}),
                "ccx_filename": ("STRING", {"default": config.get("ccx_filename", "sd-ppp_PS.ccx"), "placeholder": "CCX文件名(含后缀)"}),
                "auto_run_on_restart": (["enable", "disable"], {"default": "enable" if config.get("auto_run_on_restart", False) else "disable"}),
            },        
        }
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                "source_path": ("STRING", {"default": config.get("source_path", ""), "placeholder": "CCX文件所在路径"}),
                "target_path": ("STRING", {"default": config.get("target_path", ""), "placeholder": "目标解压路径"}),
                "ccx_filename": ("STRING", {"default": config.get("ccx_filename", "sd-ppp2_PS.ccx"), "placeholder": "CCX文件名(含后缀)"}),
                "auto_run_on_restart": (["enable", "disable"], {"default": "enable" if config.get("auto_run_on_restart", False) else "disable"}),
            },        
        }
