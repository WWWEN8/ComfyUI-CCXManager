...
class CreateSDPPPInstallationDirectory:
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                "base_directory": ("STRING", {"default": default_base_directory, "placeholder": "基础目录路径，例如：C:/Program Files/Adobe/Adobe Photoshop 2023/Plug-ins"}),
                "auto_run_on_restart": (["enable", "disable"], {"default": "enable"}),
            },
        }
    ...
    def create_directories(self, base_directory, auto_run_on_restart):
        try:
            # 更新配置
            self.config["base_directory"] = base_directory
            ...
            status_msg = f"成功: 已在 {base_directory} 下创建文件夹: {', '.join(folders_to_create)}"
...
# 首先运行目录创建节点（优先级最高）
print(f"[CCXManager] 优先执行目录创建节点的自动运行...")
